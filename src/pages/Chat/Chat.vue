      ...
      <div id="tab_bar">
        <div class="tab-bar-left">
          ...
          <div class="select-all">
            合计：<span class="total-price">&yen;{{total}}</span>
          </div>
        </div>
        <div class="tab-bar-right">
          <a href="#" class="pay" @click.prevent="buyOrder">去结算</a>
        </div>
      </div>
    </div>
    <select-login v-else/>
...
<script>
  ...
  export default {
    ...
    methods:{
      updateGoodsCount(goods,isAdd){//实现商品单击增加和减少
      ...
      singleGoodsSelected(goods){//单个商品的选中
        this.$store.dispatch('singleGoodsSelected',{goods});
        this.getGoodsprice();
        this.hasSelectedALL();
      },
      hasSelectedALL(){ //判断是否全选中状态
        let flag = true;
        this.cartgoods.forEach((goods,index)=>{
          if(!goods.checked){
            flag = false;
